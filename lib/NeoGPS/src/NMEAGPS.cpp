...
NMEAGPS::decode_t NMEAGPS::parseCommand(const msg_table_t *msgs, uint8_t cmdCount, char c)
{
  for (;;)
  {
  ...
  if (sat_count < NMEAGPS_MAX_SATELLITES)
  {
    if (fieldIndex >= 4)
    {
      ...
      case 0:
        ...
        break;
      case 1:
        parseInt(satellites[sat_count].elevation, chr);
        break;
      case 2:
        if (chr != ',')
...
bool NMEAGPS::parseZDA(char chr)
{
  ...
  case 2:
    ...
    {
      uint8_t days =
          pgm_read_byte(&NeoGPS::time_t::days_in[m_fix.dateTime.date]);
...
bool NMEAGPS::parseHeading(char chr)
{
  ...
  if (parseFloat(m_fix.hdg, chr, 2))
  {
    ...
    if (validateFields() && m_fix.valid.heading && (negative || (m_fix.hdg.whole >= 360)))
    {
      sentenceInvalid();
    }
    else
...
bool NMEAGPS::parse_lat_err(char chr)
{
  ...
  {
    if (validateFields() && (negative || (m_fix.valid.lat_err > MAX_ERROR_CM)))
    {
      sentenceInvalid();
    }
    else
...
bool NMEAGPS::parse_lon_err(char chr)
{
  ...
  {
    if (validateFields() && (negative || (m_fix.valid.lon_err > MAX_ERROR_CM)))
    {
      sentenceInvalid();
    }
    else
...
bool NMEAGPS::parse_alt_err(char chr)
{
  ...
  {
    if (validateFields() && (negative || (m_fix.valid.alt_err > MAX_ERROR_CM)))
    {
      sentenceInvalid();
    }
    else
